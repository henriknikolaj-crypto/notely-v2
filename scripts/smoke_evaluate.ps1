...
Write-Host "[SMOKE] evaluate ($BaseUrl)" -ForegroundColor Cyan

$payload = @{ question="Testspørgsmål"; answer="Testsvar"; includeBackground=$false } | ConvertTo-Json
$resp = $null; $code = $null
try {
  $resp = Invoke-WebRequest -Uri "$BaseUrl/api/evaluate" -Method POST -ContentType 'application/json' -Body $payload -ErrorAction Stop
